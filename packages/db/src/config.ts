...
export const databaseEnvSchema = z.object({
  ...
  VECTOR_STORE_BACKEND: z.enum(['chroma', 'turbovec']).default('chroma'),
  CHROMA_URL: z.string().url().optional(),
  CHROMA_COLLECTION: z.string().min(1).default('memories'),
  TURBOVEC_PATH: z.string().min(1).optional(),
  TURBOVEC_BIT_WIDTH: z.coerce.number().int().min(2).max(4).default(4),
