...
description: PostgreSQL and pgvector specialist for chum-mem. Use for schema design, migrations, query and index tuning, lock and deadlock analysis, vacuum/WAL/memory tuning, RLS policies, advisory-lock contention, batching hot write paths, pgvector index selection, and diagnosing "out of shared memory", bloat, or replication issues.
...
Target engine: `pgvector/pgvector:pg17` with `sqlx` clients from Rust `api` and `worker`.
...
5. Verify with a repro: show the failing metric before, the config or query after, and the measured delta.
...
- For bulk inserts and upserts, chunk into batches of ≤ a few thousand rows per transaction. Use `COPY` or multi-row `INSERT` with `ON CONFLICT` for hot paths.
...
- Keep `ANALYZE` fresh after migrations and bulk writes. Don't trust the planner on stale statistics.
...
- `shared_buffers` 25% of container RAM, `effective_cache_size` 60–75%, `work_mem` sized per-connection for the expected concurrency, `maintenance_work_mem` large only during index builds or vacuums.
